...
def dashboard():
    ...
    if 'gender' in df.columns and df['gender'].notna().any():
        ...
        gender_fig = px.bar(gender_stats, x='gender', y='count', color='label',
                            ...
                            barmode='group',
                            color_discrete_map={'Normal': "#35727B", 'Fraude': "#6C1995"})
        # تطبيق خلفية زرقاء داكنة
    ...
    # GRAPH 3: LINE CHART - Count of predictions over time - ألوان واضحة
    ...
    # Customize appearance - خلفية زرقاء وألوان واضحة
...
def analysis():
    ...
    if request.method == 'POST':
        ...
        if fig:
            # تطبيق خلفية زرقاء داكنة على جميع الرسوم البيانية
            fig.update_layout(
                paper_bgcolor='#0a1929',
                plot_bgcolor='#0a1929',
                font=dict(color='#ffffff'),
...
def user_analysis():
    ...
    pie_fig = px.pie(values=fraud_count.values, names=labels, 
                     title="Répartition des Prédictions (0=Normal, 1=Fraude)",
                     color_discrete_sequence=["#35727B", "#6C1995"])
    pie_fig.update_layout(
        paper_bgcolor='#0a1929',
        plot_bgcolor='#0a1929',
        font=dict(color='#ffffff'),
    ...
    # Box plot - ألوان واضحة
    box_fig = px.box(df, x='prediction', y='amt', 
                     title='Distribution des montants par prediction', 
                     labels={'prediction': 'Prediction (0=Normal,1=Fraude)'},
                     color='prediction',
                     color_discrete_map={0: "#35727B", 1: "#6C1995"})
